...
namespace proyControlInv
{
    class Ruta
    {
        ...
        public string recorrido(string nombre, int horaInicio, int horaFin)
        {
            ...
            int tiempo = 0;
            recorrido = temp.nombre + " " + horaInicio.ToString();
            while (tiempo < horaFin - horaInicio)
            {
                temp = temp.quienSigue;
                tiempo += temp.minutos;
                recorrido += Environment.NewLine + Environment.NewLine + temp.nombre + " " + temp.minutos.ToString();
            }
            return recorrido;
        }
